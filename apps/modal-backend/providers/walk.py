...
def keyframe_instruction(shot: WalkShot, medium: str | None = None, styled: bool = False) -> str:
    ...
    return (
        ...
        f"sky. {describe(shot, blocks=True)} The block colours only say which block is "
        "which place: never paint a wall or roof in its block colour. Paint each block as "
        "the place it is, in its own described materials and colours, keeping its "
        "position and size, and fill the rest with plain street, cobbles and sky. Do not "
        "add any other named building, tower or landmark. "
