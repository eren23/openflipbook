...
async def click_to_subject(
    ...
    if world_mode:
        world_clause = (
            ...
            "shop, workshop) OR a discrete roofed building you would step into "
            "next (a tower, house, temple, church, keep, windmill, lighthouse, "
            "conservatory, dome, glasshouse, hall). Judge the FORM of the "
            "SUBJECT you named in (1) itself, not everything else visible "
            "in the crop: a SINGLE roofed building — however grand, even "
            "when other buildings surround it — is \"interior\". "
            "\"complex\" = the subject ITSELF is multiple separate "
            "structures or a walkable outdoor area (a castle compound, "
            "campus, harbor, market square, "
            "village); \"landscape\" = open terrain (a valley, coastline, "
            "forest); \"generic\" = none of these / unclear. When the PAGE "
            "ITSELF shows an interior, a doorway, archway, staircase, "
            "corridor or hatch is \"interior\" too — it leads to the next "
            "enclosed space the reader would step into. This picks the "
...
async def precompute_click_candidates(
    ...
    system = (
        ...
        "conservatory, dome, glasshouse, hall). Judge the FORM of each "
        "subject itself, not everything else visible around it: a SINGLE "
        "roofed building — however grand, even when other buildings surround "
        "it — is \"interior\". \"complex\" = the subject ITSELF is multiple "
        "separate structures or a walkable outdoor area (a castle compound, "
        "campus, harbor, market square, village); \"landscape\" = "
        "open terrain (a valley, coastline, forest); \"generic\" = none of "
        "these / unclear. When the PAGE ITSELF shows an interior, doorways, "
        "archways, staircases, corridors and hatches are PRIME subjects — "
        "classify them enter_as \"scene\", place_form \"interior\" (each "
        "leads to the next enclosed space); loose furniture and objects stay "
        "\"generic\" unless the reader would walk INTO them. If the page is "
        "a scenic illustration or a map "
